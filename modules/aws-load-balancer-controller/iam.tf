    ...
    "Statement": [
        {
            ...
            "Condition": {
                "StringEquals": {
                    "oidc.eks.region-code.amazonaws.com/id/${local.oidc}:aud": "sts.amazonaws.com",
                    "oidc.eks.region-code.amazonaws.com/id/${local.oidc}:sub": "system:serviceaccount:kube-system:aws-load-balancer-controller"
                }
            }
        }
    ]
